  ...
  <div class="flex justify-between">
    ...
    <div class="border p-5 border-gray-200 rounded-xl">
      ...
      <fieldset class="fieldset">
        ...
        <input v-model="params.to" type="text" class="input validator w-full" placeholder="Type here" pattern="^[1-9A-HJ-NP-Za-km-z]{43,44}$" required />
      ...
      <fieldset class="fieldset">
        ...
        <input type="number" class="input validator w-full" placeholder="Type a number" v-model="params.amount" />
        <p class="validator-hint">Must be between be 1 to 10</p>
      </fieldset>

      <button class="btn btn-primary w-full mt-3" :disabled="loadingTransfer" @click="transfer()">
        <span v-if="loadingTransfer" class="loading loading-spinner"></span>
        <p>Transfer</p>
      </button>
    </div>

    <div class="mockup-code w-120">
...
let buttonListen: (() => void) | null = null;
...
onUnmounted(() => {
  if (buttonListen) buttonListen();
  if (infoListen) infoListen();
});
</script>